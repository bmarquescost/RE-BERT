...
def main():
    ...
    apps = ['Evernote', 'Facebook', 'Netflix', 'PhotoEditor', 'Spotify', 'Twitter', 'WhatsApp', 'eBay']
    if opt.model_trained_on not in apps:
        print(f"Invalid app: no model trained on such app named {app}")
    ...
    for app in apps:
        ...
        output_file_name = f"{opt.model_trained_on}_model_on_{app}"
        f_out = open(output_file_name, "w")
        for feature, confidence in zip(features_extracted, confidences):
            f_out.write(f"{feature},{confidence}\n")
        f_out.close()
        
        print('Extracted software requirements --> '+ output_file_name)
